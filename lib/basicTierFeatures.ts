...
export interface CryptoEvent {
  ...
  date: string;
  coin: string;
  category: 'launch' | 'airdrop' | 'hardfork' | 'conference' | 'listing' | 'upgrade';
  url?: string;
  description?: string;
}
...
// Monad ecosystem snapshot
export interface MonadSnapshot {
  week: number;
  subsCount: number;
  txCount: number;
  topContracts: Array<{
...
// Get tip of the week
export async function getTipOfTheWeek(): Promise<string> {
  ...
  const tipIndex = currentWeek % cryptoTips.length;
  const tip = cryptoTips[tipIndex];
...
export async function saveUpcomingEvents(events: CryptoEvent[]): Promise<void> {
  const db = await getDb();
  ...
  // Insert new events
  if (events.length > 0) {
    await db.collection('upcoming_events').insertMany(
      events.map(event => ({
        ...event,
        createdAt: new Date()
      }))
...
// Save news digest
export async function saveNewsDigest(articles: NewsArticle[]): Promise<void> {
  const db = await getDb();
  ...
  if (articles.length > 0) {
    await db.collection('news_digest').insertMany(
...
// Get news digest
export async function getNewsDigest(): Promise<NewsArticle[]> {
  const db = await getDb();
...
export async function saveMonadSnapshot(snapshot: Omit<MonadSnapshot, 'timestamp'>): Promise<void> {
  const db = await getDb();
  
  const fullSnapshot: MonadSnapshot = {
    ...snapshot,
    timestamp: new Date()
  };
...
export async function getLatestMonadSnapshot(): Promise<MonadSnapshot | null> {
  const db = await getDb();
...
export async function getBasicTierData(): Promise<{
  tip: string;
  events: CryptoEvent[];
  news: NewsArticle[];
  monadSnapshot: MonadSnapshot | null;
}> {
