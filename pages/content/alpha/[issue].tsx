    ...
    const run = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          // Check if user has tier 2 or higher (Premium or VIP)
          if (data.tier >= 2) {
            setAllowed(true)
            setStatus('')
            // Fetch latest alpha content
            try {
              const alphaRes = await fetch(`/api/alpha?address=${user}`)
              if (alphaRes.ok) {
                const alphaData = await alphaRes.json()
                setAlpha(alphaData)
              }
            } catch (alphaErr) {
              console.error('Failed to fetch alpha content:', alphaErr)
            }
          } else {
            setStatus('Access denied. Premium subscription required for Alpha content.')
            setAllowed(false)
          }
        }
      } catch (e) {
